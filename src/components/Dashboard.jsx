...
export default function Dashboard(props) {
    ...
    for(let key in props.locationInfo.forecast) {
        days.push(
            <div key={key} className='flex justify-center items-center gap-4 mb-4'>
                <p>{!props.locationInfo.forecast[key].date ? '~' : props.locationInfo.forecast[key].date}</p>
                <p>{!props.locationInfo.forecast[key].max ? '~' : Math.round(props.locationInfo.forecast[key].max)} °F / {!props.locationInfo.forecast[key].min ? '~' : Math.round(props.locationInfo.forecast[key].min)} °F</p>
                <p>{!props.locationInfo.forecast[key].description ? '~' : props.locationInfo.forecast[key].description}</p>
            </div>
        )
    }
    ...
    return (
        <div>
            ...
            {/* MAIN DASHBOARD */}
            <div className='grid grid-cols-2 mt-5 mx-auto p-5 bg-gray-500 w-3/5 rounded-2xl'>
